...
(function () {
    ...
    function workerSleep(ms) {
        return new Promise(function (resolve) {
            try {
                var blob = new Blob(['onmessage=function(e){setTimeout(function(){postMessage(1)},e.data)}'], { type: 'application/javascript' });
    ...
    function workerTick(intervalo, condicao, timeout, callback) {
        ...
        var w = null;
        try {
            var blob = new Blob(['onmessage=function(e){setInterval(function(){postMessage(1)},e.data)}'], { type: 'application/javascript' });
        ...
        if (w) { w.onmessage = tick; w.postMessage(intervalo); }
        else {
     ...
     * PERSISTÊNCIA SEGURA
     * ---------------------------------------------------------------------
     * O estado em memória mantém as imagens (base64) para a exportação; o
     * que vai para o IndexedDB é uma CÓPIA sem data:image embutidos,
     * mantendo o JSON persistido compacto. As gravações são enfileiradas e
     * executadas em ordem, para que uma transação antiga nunca sobrescreva
     * um estado mais novo. IndexedDB é melhor esforço (não bloqueia o fluxo).
     * =================================================================== */
    ...
    function salvarEstadoIdb(json) {
        ...
        idbFila = idbFila.then(function () {
            return idbOperacao('readwrite', function (store) {
                store.put({ key: CONFIG.storageKey, salvoEm: Date.now(), json: json });
            });
        }).catch(function (e) {
        ...
        return idbAbrir().then(function (db) {
            return new Promise(function (resolve, reject) {
                try {
                    ...
                    var req = store.get(CONFIG.storageKey);
                    req.onsuccess = function () {
                        ...
                        try {
                            ...
                            if (parsed && typeof parsed === 'object' && parsed.biblioteca) resolve(parsed);
                            else resolve(null);
    ...
    function estadoVazio() {
        return {
            plano: null,
            config: null,
            status: 'parado',          // parado | rodando | pausado | completo | erro
            fase: 'nenhuma',           // pasta-check | filtros | criando | coletando
            modo: 'lote',              // lote | sob-demanda
            planIndex: 0,              // índice da matéria atual no plano
            loteInicio: 0,
            loteFim: 0,
            cadernoAtual: null,        // {id, titulo, total, coletadas, ultimaPosicao}
            biblioteca: {},            // { [cadernoId]: {id, titulo, total, questoes: [...], completo} }
            impressao: { data: '', usadas: 0 },
            impressaoParte: null,      // {start, count} da parte em andamento
            mensagem: '',
            erro: null,
            retomada: false,
            atualizadoEm: null
    ...
    /* Restaura o estado do IndexedDB ANTES de criar a UI, renderizar o
     * status e disparar o auto-resume. Se o IndexedDB estiver indisponível
     * ou a leitura falhar, mantém o estado vazio (em memória) e avisa —
     * sem fallback para outro storage. */
    function carregarEstado() {
        return carregarEstadoIdb().then(function (parsed) {
            ...
            console.warn('[TecFabrica] AVISO: IndexedDB indisponível ou falhou a leitura — iniciando com estado vazio (' + (e && e.message || e) + ').');
            return null;
        });
    }
    ...
    function salvarEstado() {
        ...
        try {
            ...
            estado.mensagem = 'Erro: não foi possível salvar o estado. Exporte os cadernos e limpe os dados.';
            if (UI && typeof UI.renderProgresso === 'function') UI.renderProgresso();
            return;
        }
    ...
    async function coletarCaderno(caderno) {
        ...
        while (true) {
            ...
            var existente = porId[questao.id] ? colecao.find(function (q) { return q.id === questao.id; }) : null;
  ...
  render();
    ...
    function zipStore(files) {
        ...
        var directorySize = directory.reduce(function (t, e) { return t + e.length; }, 0);
    ...
    async function loadImageAssets(questionImages) {
        ...
        for (var qi = 0; qi < questionImages.length; qi += 1) {
            for (var ii = 0; ii < questionImages[qi].length; ii += 1) {
                var source = questionImages[qi][ii];
                if (assets.has(source)) continue;
                var asset = await readImageAsset(source);
                if (asset) { asset.mediaIndex = embedded.length + 1; embedded.push(asset); assets.set(source, asset); }
            }
        }
